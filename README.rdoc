...
And the Koans will guide you where to look in your text editor. It will tell you
the next file, and the next line containing the koan to practice.
...
The very first time you run the koans you will see the following output:
    ...
    your path thus far [X_________________________________________________] 0/164
...
Open the +about_nil.rb+ file and look at the first test:
...
the method name (+test_nil_is_an_object+) and the parts inside the method (everything
before the +end+).
...
Install the tool `entr` and then ask it to "watch" the koans for changes:

    # Mac OS: brew install entr
    # Ubuntu:  apt install entr
    # Other Linux: install entr from your favorite repository manager
